...
class DBBotRepository(DBBaseRepository):
    ...
    @property
    def _query(self):
        return f"SELECT bot_id, date_created from {self.db_schema}.bots"
    ...
    async def get_many(self, limit: int, offset: int) -> Optional[list[_model_cls]]:
        query = f"SELECT bot_id, date_created from {self.db_schema}.bots ORDER BY date_created LIMIT {limit} OFFSET {offset}"
